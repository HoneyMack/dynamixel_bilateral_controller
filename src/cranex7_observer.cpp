...
map<int, double> Cranex7Observer::step_torque_react(map<int, double> currents, map<int, double> positions, map<int, double> velocities, map<int, double> tau_d) {
    ...
    for (auto& kv : currents) {
        ...
        if (kv.first == 2) {
            ...
            const double pos2 = positions[2], pos4 = positions[4];

            const double tau_d_known = j2_tau_r_filter.filter(Ds[kv.first] * velocities[kv.first] + M1 * sin(pos2) - M2 * sin(pos2 + pos4));
            ...
            const double tau_d_known = j4_tau_r_filter.filter(Ds[kv.first] * velocities[kv.first] - M3 * sin(pos2 + pos4));
            const double tau_r = tau_d[kv.first] - tau_d_known;
            tau_rs[kv.first] = tau_r;
        }
        else {
